...
import { getAssessmentData, getMarketResearchQuestions } from "@/lib/questions";
import type {
  AssessmentResult,
  MaturityAnswers,
...
function ResultsContent() {
  ...
  useEffect(() => {
    if (isPreExplorer) return;

    const stored = sessionStorage.getItem("assessmentAnswers");
    if (!stored) return;

    const answers: MaturityAnswers = JSON.parse(stored);
    const calcResult = calculateResults(answers);
    setResult(calcResult);

    // Submit to API and store identifier for follow-up submissions
    fetch("/api/submit", {
      method: "POST",
  ...
  if (isPreExplorer) {
    ...
    const fallback = data.screening.fallbackResult;
    return (
  ...
  if (phase === "market-research") {
    const mrQuestions = getMarketResearchQuestions();
    return (
      <main className="min-h-screen flex items-center justify-center px-4 py-12">
        <div className="max-w-xl w-full animate-fade-in">
          <MarketResearchForm
            ...
            onComplete={(answers) => {
              ...
              // Submit market research (fire and forget)
              fetch("/api/submit", {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
      ...
      <div className="max-w-2xl w-full animate-fade-in">
        ...
        <div className="mb-10">
          ...
          <div className="space-y-3">
            {result.lowestCapabilities.map((cap) => (
              <div
                key={cap.id}
              ...
              >
                <div className="text-sm font-medium mb-1">
                  {cap.name}{" "}
                  <span className="text-muted">({cap.score}/100)</span>
                </div>
                <p className="text-sm text-muted">
                  {getGrowthRecommendation(cap.id)}
                </p>
              </div>
            ))}
          </div>
